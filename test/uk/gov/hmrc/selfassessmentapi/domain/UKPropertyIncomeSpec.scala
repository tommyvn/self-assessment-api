...
import uk.gov.hmrc.selfassessmentapi.domain.UKPropertyIncomeType._

class UKPropertyIncomeSpec extends JsonSpec {

  "format" should {

    "round trip valid UKPropertyIncome json" in {
      roundTripJson(UKPropertyIncome(`type` = RentIncome, amount = BigDecimal(1000.99)))
    }
  }
  ...
  "validate" should {
    "reject amounts with more than 2 decimal values" in {
      Seq(BigDecimal(1000.123), BigDecimal(1000.1234), BigDecimal(1000.12345), BigDecimal(1000.123456789)).foreach { testAmount =>
        val value = UKPropertyIncome(`type` = RentIncome, amount = testAmount)
        assertValidationError[UKPropertyIncome](
          value,
    ...
    "reject negative amount" in {
      val seIncome = UKPropertyIncome(`type` = RentIncome, amount = BigDecimal(-1000.12))
      assertValidationError[UKPropertyIncome](
        seIncome,
        Map(("/amount", INVALID_MONETARY_AMOUNT) -> "amount should be non-negative number up to 2 decimal values"),
