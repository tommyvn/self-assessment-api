...
class FurnishedHolidayLettingsBalancingChargeSpec extends JsonSpec {

  "format" should {
    "round trip FurnishedHolidayLettingsBallancingCharge json" in {
        roundTripJson(FurnishedHolidayLettingsBalancingCharge(amount = BigDecimal(1000)))
    }
  }
  ...
  "validate" should {
    "reject amounts with more than 2 decimal values" in {
      Seq(BigDecimal(1000.123), BigDecimal(1000.12456), BigDecimal(1000.123454), BigDecimal(1000.123456789)).foreach { testAmount =>
        val expense = FurnishedHolidayLettingsBalancingCharge(amount = testAmount)
        assertValidationError[FurnishedHolidayLettingsBalancingCharge](
          expense,
    ...
    "reject negative amount" in {
      val expense = FurnishedHolidayLettingsBalancingCharge(amount = BigDecimal(-1000.13))
      assertValidationError[FurnishedHolidayLettingsBalancingCharge](
        expense,
        Map(("/amount", INVALID_MONETARY_AMOUNT) -> "amount should be non-negative number up to 2 decimal values"),
